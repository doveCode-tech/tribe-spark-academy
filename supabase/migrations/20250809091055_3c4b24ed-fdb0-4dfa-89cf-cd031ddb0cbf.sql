...
CREATE POLICY IF NOT EXISTS "Admins can manage course_tutors"
ON public.course_tutors
FOR ALL
USING (public.is_admin(auth.uid()))
WITH CHECK (public.is_admin(auth.uid()));

CREATE POLICY IF NOT EXISTS "Tutors can view own course assignments"
ON public.course_tutors
FOR SELECT
...
CREATE POLICY IF NOT EXISTS "Tutors/Admins can manage code_templates"
