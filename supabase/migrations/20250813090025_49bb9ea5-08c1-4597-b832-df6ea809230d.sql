-- Idempotent migration to support project submissions and safe enrollment requests
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'storage' AND tablename = 'objects' AND polname = 'Public read lesson videos'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'storage' AND tablename = 'objects' AND polname = 'Tutors/Admins upload lesson videos'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'storage' AND tablename = 'objects' AND polname = 'Students manage own project files'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'storage' AND tablename = 'objects' AND polname = 'Tutors/Admins can view project submissions'
  ) THEN
